"""
Core task operations that can be used by both MCP tools and ChatService.
These are regular async functions without MCP decorators.
"""
...
async def list_tasks_operation(user_id: int, status: str = "all") -> list[dict]:
...
async def complete_task_operation(task_id: int, user_id: int) -> dict:
...
async def delete_task_operation(task_id: int, user_id: int) -> dict:
