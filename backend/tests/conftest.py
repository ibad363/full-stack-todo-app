...
# Add backend folder to path so `import modules.*` resolves correctly
...
# Provide minimal env vars required by Settings used in modules.core.config
...
import modules.core.database as db_module
db_module.engine = test_engine
# Import models so SQLModel metadata includes all table definitions
import modules.models  # registers `User` and `Task` models
...
from modules.api.auth import limiter as auth_limiter
